...
class TestImage(TestCase):
    ...
    def test_levelUp0(self):
        self.user1.tagImage('a', self.image)  # has score 0
    ...
    def test_levelUp1(self):
        ...
        self.user2.tagImage('a', self.image)  # has score 1
        self.user3.tagImage('a', self.image)
        self.assertEqual(self.image.level, 1)

        self.assertEqual(self.user2.getScore(), 1)
        self.user2.tagImage('a', self.image)  # adds already known tag
        self.assertEqual(self.user2.getScore(), 1 + 2)  # -> score + 2
        self.user2.tagImage('b', self.image)  # adds new tag
    ...
    def test_levelUp2(self):
        ...
        self.user5.tagImage('a', self.image)
        self.assertEqual(self.image.level, 2)

        self.assertEqual(self.user4.getScore(), 2)
        self.user4.tagImage('a', self.image)  # adds already known tag
        self.assertEqual(self.user4.getScore(), 2 + 2)  # -> score + 2
        self.user4.tagImage('c', self.image)  # adds new tag
